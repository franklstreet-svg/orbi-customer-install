...
def legal_draft_document():
    ...
    system = (
        ...
        "End every document with: 'Prepared by myVola AI Paralegal — for attorney "
...
def public_chat():
    ...
    # BUSINESS-NAME GUESS SCRUB (added 2026-06-22, revised 2026-06-22):
    # Frank's directive: USE what the scrape returned — state business name
    # confidently if you have it, only ask for what's missing. The earlier
    # bug was hedging ("is it likely X?", "maybe something like Y?") and
    # inventing variations from the URL. Only strip those weasel phrases;
    # let stated facts from a successful scrape survive.
    if (resp and resp.text):
        try:
            _l = resp.text.lower()
            if "business name" in _l or "biz name" in _l:
                _patterns = [
                    # Hedge / guess phrasings — strip these
                    r"\s*(is\s+it\s+(likely|the\s+same\s+as)\s+[^.?!\n]*[.?!]?)",
                    r"\s*,?\s*maybe\s+something\s+like\s+['\"][^'\"]+['\"][^.?!\n]*[.?!]?",
                    r"\s*,?\s*perhaps\s+['\"][^'\"]+['\"][^.?!\n]*[.?!]?",
                    r"\s*,?\s*likely\s+['\"][^'\"]+['\"][^.?!\n]*[.?!]?",
                    r"\s*,?\s*(or\s+does\s+it\s+go\s+by\s+something\s+else[?.!]?)",
                    # "but I want to confirm that with you" — drops the
                    # weak "confirm" verbiage but keeps the asserted name
                    r"\s*,?\s*but\s+I\s+want\s+(?:to\s+)?(?:confirm|verify)[^.?!\n]*[.?!]?",
                    r"\s*,?\s*but\s+I'?d?\s+like\s+to\s+(?:confirm|verify)[^.?!\n]*[.?!]?",
                    r"\s*so,?\s+to\s+confirm[\s:,]+",
                ]
                _orig = resp.text
                ...
                if _orig != resp.text:
                    log.info("chat: scrubbed business-name hedge phrases")
        except Exception as _e:
...
def owner_setup_initial_password():
    ...
    assistant_name = (data.get("assistant_name") or "").strip()[:32] or "Vola"
    if len(new_pw) < 6:
...
def _needs_onboarding() -> bool:
    ...
    frank_install_sentinels = {"myVola", "Vola", "myOrbi", "Orbi", "My Orbi AI Solutions", "My Orby AI Solutions"}
...
def _try_onboarding(message: str, user_rec: dict) -> dict | None:
    ...
    if not state.get("welcome_shown"):
        ...
        hello = f"Hi{', ' + owner_first if owner_first else ''}! I'm Vola — your new assistant."
            ...
            if low in keep_signals:
                ...
                for prefix in ("call you ", "call me ", "name you ",
                    ...
                    if cleaned.lower().startswith(prefix):
                        ...
                        break
                collected["assistant_name"] = cleaned[:32].strip() or "Vola"
        elif key == "website":
    ...
    try:
        biz = mod_business.load(DATA_DIR) or {}
        personality = biz.get("personality") or {}
        assistant_name = (collected.get("assistant_name") or "Vola").strip() or "Vola"
        personality["name"] = assistant_name
